...
#Hacemos un bucle para correr todos los tickers que hemos pedido
    ...
    for e in range(n):
        try:
            financial_ratios_aux=financial_ratios[e]
            financial_df=pd.DataFrame.from_dict(financial_ratios_aux, orient='index')
            financial_df = financial_df.transpose()
            financial_df["extraction_date"]=today 
            financial_ratios_df=financial_ratios_df.append(financial_df)  
        except:
            pass
