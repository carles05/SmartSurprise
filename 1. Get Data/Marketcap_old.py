...
symbol_list_2=symbol_list_df[2062:len(symbol_list_df)] #desde TCS
...
date_df=pd.read_sql_table('M_Quarters',con=engine)
#Chequamos que no haya ningun duplicado
date_df=date_df.Quarter.unique()
...
for a in range(len(ticker_list)):
    ...
    n=len(marketcap)
    marketcap_dat_df=pd.DataFrame()
    for e in range(n):
        try:
            marketcap_aux=marketcap[e]
            marketcap_df=pd.DataFrame.from_dict(marketcap_aux, orient='index')
            marketcap_df = marketcap_df.transpose()
            marketcap_df["extraction_date"]=today
            if marketcap_df.date[0] in date_df:
                marketcap_dat_df=marketcap_dat_df.append(marketcap_df)  
        except:
            pass
    
    try:
        marketcap_dat_df.to_sql('ST_Marketcap', con = engine, if_exists = 'append',index=False)
    except:
        pass
